...
class DataView:
    ...
    def __get_binary(self, start_index, byte_count, signed=False, byteorder='little'):
        integers = [self.array[start_index + x] for x in range(byte_count)]
        bytes = [integer.to_bytes(
            self.bytes_per_element, byteorder=byteorder, signed=signed) for integer in integers]
    ...
    def get_uint_32(self, start_index, byteorder='little'):
        bytes_to_read = 4
        return int.from_bytes(self.__get_binary(start_index, bytes_to_read), byteorder=byteorder)
